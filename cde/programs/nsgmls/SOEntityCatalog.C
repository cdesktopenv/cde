 ...
 */
/* $XConsortium: SOEntityCatalog.C /main/1 1996/07/29 17:03:30 cde-hp $ */
					 ...
					 Messenger &mgr) const
{
  ...
  while (maps.size() > 0) {
    ...
    StringC s;
    if (maps.back().type == ParsedSystemIdMap::catalogDocument) {
	...
	return 0;
      ...
      ASSERT(maps.back().type == ParsedSystemIdMap::catalogPublic);
						  ...
						  Messenger &mgr) const
{
  ...
  if (!impl->entityManager()->parseSystemId(sysid, charset, 0, 0, mgr, v))
    return;
  if (v.maps.size() > 0) {
    if (v.maps[0].type == ParsedSystemIdMap::catalogDocument) {
